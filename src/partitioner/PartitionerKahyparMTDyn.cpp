 ...
 */
PartitionerKahyparMTDyn::PartitionerKahyparMTDyn(Config &config, unsigned maxNodes, unsigned maxEdges,
...
/**
   Destructor.
 */
PartitionerKahyparMTDyn::~PartitionerKahyparMTDyn() {
  mt_kahypar_free_context(context);
} // destructor

void PartitionerKahyparMTDyn::initPartitioner(Config &config) {
 ...
 */
void PartitionerKahyparMTDyn::computePartition(HyperGraph &hypergraph, Level level,
  ...
  auto hypergraph_rs = hypergraph::with_capacity(hypergraph.getSize());
  ...
  for (auto &edge : hypergraph) {
    ...
    hypergraph_rs->add_net(edge_vector);
  ...
  if (clustering_method == "min") {
    block_count = hypergraph_rs->incidence_graph_louvain_num_min();
  } else {
    block_count = hypergraph_rs->incidence_graph_louvain_num();
  }
  ...
  mt_kahypar_set_partitioning_parameters(context, block_count /* number of blocks */,
  ...
  auto hgraph =
      mt_kahypar_create_hypergraph(preset, num_vertices, num_hyperedges,
